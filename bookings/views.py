            ...
            if request.method == "POST":
                ...
                if start < str(date.today()):
                    ...
                    data = Bookinghotel(firstname=name, lastname=last,
                                        email=email, contact_no=contact, no_people=person, username=username, useremail=useremail, no_rooms=room, userpassword=password, start=start, end=end, hotelname=hotelname, city=hotelcity, state=hotelstate, current_cost=hotelcost)
                    data.save()
                    url = "/dashboard/{}".format(id)
